...
def _test():
    ...
    x_old = torch.tensor([[0.03 * x * y for y in range(8)] for x in range(8)], dtype=dtype, device=device) 
    c = 100
    x_new = x_old * c
    print(x_new)
    ...
    ----------------------------------------------------------------------------------------------------'''
    #STEP 1: Blocking
    blocked = compressor.block(x_old)
    blocked_new = compressor.block(x_new)
    for i in range(0,blocked[0].size()[0]):
        ...
        #STEP 2.1: Find the new information required for multiplication of constant
        first_element = c * first_element
        ...
        print((blocked_new[0, i] - inverse_normalized_part2).norm(torch.inf)) #norm error
