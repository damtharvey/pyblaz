...
def _test():
    ...
    for i in range(len(dot_row_col)):
        print(f"difference between actual dot product and dot product in compressed domain = {(dot_row_col[i] - decompressed_dotproduct[i]).norm(torch.inf)}")
        print(f"difference between actual dot product and dot product after decompressing the data = {(dot_row_col[i] - dotproduct_compression_decompression[i]).norm(torch.inf)}")
