...
def _test():
    ...
    # f(x,y) = x*y stepping 0.1 for range of values between 0 and 1
    x1 = torch.tensor([[0.03 * x * y for y in range(8)] for x in range(8)], dtype=dtype, device=device) 
    x2 = torch.tensor([[0.02 * x * y for y in range(8)] for x in range(8)], dtype=dtype, device=device) 

    x = x1@x2 #dot product of two matrices without compression
    ...
    ----------------------------------------------------------------------------------------------------'''
    #STEP 1: Blocking
    blocked1 = compressor.block(x1)
    blocked2 = compressor.block(x2)
    blocked = compressor.block(x)
    
    

    for i in range(0,blocked1[0].size()[0]):
        print(blocked[0,i])  #Printing the block so that we can compare it later with our output
        

        #STEP 2: Normalization of individual matrices
        first_element1, mean_slope1, normalized_block1 = compressor.normalize(blocked1[0, i])
        first_element2, mean_slope2, normalized_block2 = compressor.normalize(blocked2[0, i])

        

        #STEP 3: DCT without quantization of individual matrices 
        ...
        DCT1 = compressor.block_transform(normalized_block1)
        DCT2 = compressor.block_transform(normalized_block2)
        

        #STEP 4: Prediction of individual matrices

        coefficient_indices1, biggest_element1 = compressor.predict(DCT1, mean_slope1)
        coefficient_indices2, biggest_element2 = compressor.predict(DCT2, mean_slope2)
        
        
        #STEP 5: Most of the values in the paper were 0 and we were getting 127 over there. This is because our binning is starting from 0
        #while in the paper its from -i to +i
        #It seemed reasonable to subtract 127 from the output to obtain the final compressed matrix
        
        compressed_matrix1 = coefficient_indices1 - 127
        compressed_matrix2 = coefficient_indices2 - 127
        ...
        #The decompression of addition of matrix is same as decompression of a matrix

        #STEP -5: Adding back the value in the matrix that we subtarcted in step 5
        added_back1 = compressed_matrix1 + 127
        added_back2 = compressed_matrix2 + 127
        
        #STEP -4: Inverting back the prediction
        inverse_predict1 = compressor.unpredict(added_back1, mean_slope1, biggest_element1)
        inverse_predict2 = compressor.unpredict(added_back2, mean_slope2, biggest_element2)
        

        #STEP -3: Inverse DCT
        inverse_DCT1 = compressor.block_transform(inverse_predict1, inverse=True)
        inverse_DCT2 = compressor.block_transform(inverse_predict2, inverse=True)

        B1 = np.zeros(compressed_matrix1.size())
        B2 = np.zeros(compressed_matrix2.size())

        for row in range(0,8):
            for col in range(0,8):
                if row == 0 and col == 0:
                    B1[row,col] = first_element1
                    B2[row,col] = first_element2
                elif row==0 and col != 0:
                    B1[row,col] = B1[0,col-1] + mean_slope1*inverse_DCT1[0,col-1]
                    B2[row,col] = B2[0,col-1] + mean_slope2*inverse_DCT2[0,col-1]
                elif row != 0 and col == 0:
                    B1[row,col] = B1[row-1,0] + mean_slope1*inverse_DCT1[row-1,0]
                    B2[row,col] = B2[row-1,0] + mean_slope2*inverse_DCT2[row-1,0]
                else:
                    B1[row,col] = (B1[row-1,col] + B1[row,col-1])/2 + mean_slope1*inverse_DCT1[row-1,col-1]
                    B2[row,col] = (B2[row-1,col] + B2[row,col-1])/2 + mean_slope2*inverse_DCT2[row-1,col-1]



        #Creating a new matrix to store the inverse DCT of product of the individual DCTs
        inverse_DCT = np.zeros(compressed_matrix1.size())
        for row in range(0,8):
            for col in range(0,8):
                res = 0.0
                for k in range(0,8):
                    res += B1[row, k] * B2[k, col]
                inverse_DCT[row, col] = res

        inverse_DCT = torch.from_numpy(inverse_DCT)
        print(inverse_DCT)
        #STEP -2: Inverse normalized matrix part 1. Here we perform the inverse of slope and multiply back the slope in the matrix
        inverse_normalized_part1 = inverse_DCT/2 
        first_element = first_element1*first_element2
        ...
        print((blocked - inverse_normalized_part2).norm(torch.inf)) #norm error
