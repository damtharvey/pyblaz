...
def _test():
    ...
    x1 = torch.tensor([[0.03 * x * y for y in range(16)] for x in range(8)], dtype=dtype, device=device) 
    x2 = torch.tensor([[0.02 * x * y for y in range(16)] for x in range(8)], dtype=dtype, device=device) 

    x = x1 - x2  #addition of two matrices without compression
    ...
    for i in range(0,blocked1[0].size()[0]):
        ...
        mean_slope = mean_slope1 - mean_slope2
        biggest_element = biggest_element1 + biggest_element2
        

        #Creating a new matrix to store the compressed addition result
        compressed_matrix = np.zeros(compressed_matrix1.size())
        ...
        for k1 in range(0,8):
            for k2 in range(0,8):
                if ((compressed_matrix1[k1,k2] + compressed_matrix2[k1,k2]) == 0):
                    compressed_matrix[k1,k2] = 0
                else:
                    compressed_matrix[k1,k2] = (compressed_matrix1[k1,k2] * compressed_matrix2[k1,k2]) / (compressed_matrix1[k1,k2] + compressed_matrix2[k1,k2])
        compressed_matrix = compressed_matrix.astype(int)
        

        #Converting the array into tensor for decompression
        compressed_matrix = torch.from_numpy(compressed_matrix)
